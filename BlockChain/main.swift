...
class Blockchain: Chain {
    ...
    @discardableResult
    func newTransaction(choice: String, uniqID: String, date: String) -> Int64 {
        let transaction = Transaction(choice: choice, uniqID: uniqID, date: date)
    ...
    func validProof(last_proof: Int64, proof: Int64) -> Bool {
        ...
        let guess_hash = guess.sha3(.sha256)
    ...
    func resolveConflicts() -> Bool {
        ...
        for node in neighbours {
            ...
            request.responseObject { (response: Result<RestResponse<ResponseData>, RestError>) in
                switch response {
                case .success(let response_data):
                    let length = response_data.body.length
                    let chain = response_data.body.chain
...
router.post("/transactions/new") {
    ...
    guard let body = request.body?.asJSON,
        ...
        let uniqID = body["uniqID"] as? String,
    ...
    print("success")
    let index = blockchain.newTransaction(choice: choice, uniqID: uniqID, date: date)
    struct ResponseData: Encodable {
        let message: String
    }
